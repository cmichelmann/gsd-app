...
function MonthView({ state, dispatch, openTask, openEvent, openAddEvent }) {
  ...
  const [filters, setFilters] = useState({ tasks: true, events: true, vacation: true, birthdays: true, holidays: true, habits: true, sport: true });
  const today = localDate();
  const now = new Date();
  ...
  const itemsByDate = useMemo(() => {
    const m = {};
    if (filters.tasks) state.tasks.forEach(t => getTaskDates(t, winStart, winEnd).forEach(d => { (m[d] = m[d] || []).push({ kind: "task", item: t }); }));
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 9, color: "var(--muted)", textAlign: "center", marginTop: 10, opacity: 0.7 }}>
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {showAddEvent && <AddEventModal defaultDate={addEventDate} onClose={() => { setShowAddEvent(false); setAddEventDate(null); }} onAdd={e => dispatch({ type: "ADD_EVENT", payload: e })} />}
