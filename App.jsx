...
.card { background: rgba(14,14,14,0.78); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border: 1px solid var(--border); border-radius: var(--r); padding: 14px; }
.card-sm { background: rgba(14,14,14,0.78); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border: 1px solid var(--border); border-radius: var(--r-sm); padding: 10px 12px; }
...
.prio-1 { background: #C04848; opacity: 0.7; }
...
.task-card { background: rgba(14,14,14,0.82); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); border: 1px solid var(--border); border-radius: var(--r-sm); padding: 11px; margin-bottom: 7px; cursor: grab; transition: all 0.12s; position: relative; }
...
.month-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 1px; background: var(--border); border-radius: var(--r-sm); overflow: hidden; }
.month-day { background: var(--s0); min-height: 72px; padding: 5px 4px; position: relative; cursor: pointer; transition: background 0.12s; }
...
.day-num { font-size: 11px; color: var(--muted); margin-bottom: 4px; font-family: 'JetBrains Mono'; }
.day-task-bar { color: #000; font-size: 9px; padding: 2px 4px; border-radius: 3px; font-weight: 600; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.3; cursor: pointer; }

.day-list-card { background: rgba(14,14,14,0.78); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border: 1px solid var(--border); border-radius: var(--r); margin-bottom: 8px; overflow: hidden; }
...
.stat-tile { background: rgba(14,14,14,0.78); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border: 1px solid var(--border); border-radius: var(--r-sm); padding: 12px; }
...
const PRIOS = [
  ...
  { id: 1, label: "Niedrig", color: "#C04848", className: "prio-1" },
];
const ENERGIES = [
...
const SHIT_SPRUECHE = [
  "SHIT DONE!", "EINE WENIGER!", "BOAH GEIL!", "KRASS, ERLEDIGT!",
  "BOOM! WEG DAMIT!", "KACKE ABGEHAKT!", "ABGESCHISSEN!", "FERTIG DER MIST!",
  "HALLELUJA!", "KNALLER!", "GET SHIT DONE!", "DURCHGEZOGEN!",
  "BÄM! ERLEDIGT!", "RAUS DAMIT!", "VOLLE KANNE GENAGELT!", "SHIT GONE!",
  "WIE EIN BOSS!", "DURCHGEKACKT!", "EINS ABGEFRÜHSTÜCKT!", "ANGENAGELT!",
];
    ...
    <div className="splash">
      ...
      <div style={{ textAlign: "center", width: "100%", maxWidth: 360 }}>
        ...
        <p className="splash-tag" style={{ marginTop: 18, marginBottom: 22 }}>
          Tasks. Termine. Sport. Habits.<br />Alles in einem. Auf den Punkt.
        </p>
    ...
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="col" style={{ gap: 12 }}>
          ...
          <button className="btn btn-ghost" onClick={() => dispatch({ type: "UPD_SETTINGS", payload: { onboarded: false } })}>🚀 Splash erneut zeigen</button>
          <div className="card-sm">
...
// VIEWS
// ════════════════════════════════════════════════════════════════════════════
function DashboardView({ state, dispatch, openTask, openPomo, setShowRitual }) {
  const today = localDate();
    ...
    <div className="view">
      ...
      {!ritualDoneToday && (
        <div className="card-lime" style={{ marginBottom: 12, cursor: "pointer" }} onClick={() => setShowRitual(true)}>
          <div className="between">
            <div><div className="stat-label" style={{ color: "#000" }}>⚡ Daily Plan</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#000", marginTop: 2 }}>Was ist heute wichtig?</div></div>
      ...
      {todayEvents.length > 0 && (
        <>
          ...
          {todayEvents.map(e => {
            const t = eventTypeOf(e.type);
            return (
              <div key={e.id} className="card-sm" style={{ marginBottom: 6, borderLeft: `3px solid ${e.color}` }}>
                <div className="between">
                  <div className="row" style={{ flex: 1, minWidth: 0 }}>
                    {e.isHoliday ? <span style={{ fontSize: 14 }}>🎉</span> : <t.icon size={14} color={e.color} />}
      ...
      {todayTasks.length > 0 && (
        <>
          ...
          {todayTasks.slice(0, 6).map(t => (
            <div key={t.id} onClick={() => openTask(t.id)} className="card-sm" style={{ marginBottom: 6, cursor: "pointer", borderLeft: `3px solid ${catOf(t.category).color}` }}>
              <div className="between">
                <div className="row" style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{t.title}</span>
                </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 1, marginBottom: 4 }}>
        {DAYS_SHORT.map(d => <div key={d} className="mono" style={{ textAlign: "center", fontSize: 10, color: "var(--muted)", padding: "4px 0" }}>{d}</div>)}
      </div>
...
export default function App() {
  ...
  if (!state.settings.onboarded) {
    return (
      <>
        <style>{CSS}</style>
        <SplashScreen onStart={() => dispatch({ type: "UPD_SETTINGS", payload: { onboarded: true } })} />
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <div className="main">
          {state.view === "dashboard" && <DashboardView state={state} dispatch={dispatch} openTask={openTask} openPomo={openPomo} setShowRitual={setShowRitual} />}
