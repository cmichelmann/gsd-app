...
function AchievementToast() {
  ...
  useEffect(() => {
    const handler = (e) => setQueue(q => [...q, e.detail]);
    window.addEventListener("gsd:achievement-unlock", handler);
    return () => window.removeEventListener("gsd:achievement-unlock", handler);
  }, []);
  useEffect(() => {
    if (current) return;
  ...
  return (
    <div style={{
      ...
      color: "#000", padding: "14px 18px", borderRadius: 14, zIndex: 9999,
      boxShadow: "0 10px 40px rgba(170,255,0,0.5)",
      animation: "achPop 0.4s cubic-bezier(0.22, 1.5, 0.36, 1)",
      maxWidth: 320, minWidth: 240,
    }}>
      <div className="row" style={{ gap: 12 }}>
        <div style={{ fontSize: 38 }}>{current.icon}</div>
        <div style={{ flex: 1 }}>
          <div className="display" style={{ fontSize: 11, color: "rgba(0,0,0,0.6)", letterSpacing: "0.05em" }}>🏆 ACHIEVEMENT FREIGESCHALTET</div>
          <div className="display" style={{ fontSize: 16, color: "#000", marginTop: 2 }}>{current.name}</div>
          <div style={{ fontSize: 11, color: "rgba(0,0,0,0.7)", marginTop: 2 }}>{current.desc}</div>
        </div>
      </div>
    </div>
  );
...
function AppInner() {
  ...
  // Achievement engine — checks after every state change, fires unlocks
  useEffect(() => {
    ...
    newly.forEach((id, i) => {
      setTimeout(() => {
        ...
        if (ach) {
          window.dispatchEvent(new CustomEvent("gsd:achievement-unlock", { detail: ach }));
        }
      }, i * 800);
    });
  }, [state.tasks, state.events, state.workouts, state.habits, state.pomodoros, state.subscriptions, state.gratitude, state.notes, state.shoppingLists, state.settings.dailyRitualDate, state.settings.featureAchievements]);
